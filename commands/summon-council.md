...
5. **Grumpy review** — run outputs through the 9-member default review panel (of 12 reviewers total; privacy/i18n/budget tier in on relevance), fanning out per the reconcile-after-return policy loop in `.claude/rules/fan-out-policy.md` (concurrency-aware batching B1, graceful degradation with one re-dispatch B2, and the liveness floor):
