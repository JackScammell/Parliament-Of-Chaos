...
5. **Grumpy review** — run outputs through the 9-member default review panel (of 12 reviewers total; privacy/i18n/budget tier in on relevance), fanning out per the reconcile-on-notification policy loop in `.claude/rules/fan-out-policy.md` (detached dispatch tallied via completion notifications; disk-verified prompts B7; explicit verdicts B6; concurrency-aware batching B1; graceful degradation with one fresh re-dispatch at terminal state B2; and the liveness floor — never conclude while a member's task is live):
...
- Parallel fan-out reliability has a Claude Code version floor (non-cascading sibling failures) — see the *Parallel fan-out version floor* section of `.claude/rules/fan-out-policy.md` rather than restating the version here. How non-reporting members are detected and recovered (batching, one re-dispatch, liveness floor, `INCOMPLETE` on floor non-report) is single-sourced in the same file.
