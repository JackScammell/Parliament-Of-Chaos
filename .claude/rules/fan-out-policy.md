...
> `log_event.sh` writes it and `settings.json` wires it — shipped as of v1.23.0. The one residual
> caveat: on a Claude Code build old enough that the `TaskCompleted` hook is not fired, Failed vs
